...
export const PASSIVES = [
    {
        ...
        desc: '+2 к максимуму HP (и текущему)',
        icon: '❤',
        apply: p => {
            p.maxTorsoHP += 2;
            p.torsoHP = Math.min(p.maxTorsoHP, p.torsoHP + 2);
        }
    },
    {
        id: 'damage',
        name: 'БОЕВОЙ ЧИП',
        desc: '+1 ко всему урону',
        icon: '⚔',
        apply: p => {
            p.bonusDamage += 1;
        }
    ...
    {
        id: 'speed',
        name: 'УСКОРЕННЫЙ ПРИВОД',
        desc: '+15% к скорости передвижения',
        icon: '⚡',
        apply: p => {
            p.speedBonus = (p.speedBonus || 0) + 0.15;
        }
    },
    {
        id: 'swingSpeed',
        name: 'ГИРОСКОП',
        desc: '+15% к скорости замаха',
        icon: '🌀',
        apply: p => {
            p.swingSpeedBonus = (p.swingSpeedBonus || 0) + 0.15;
        }
    }
];
